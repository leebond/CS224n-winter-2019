...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':
            ...
            w = self.stack.pop(-2)
            self.dependencies.append((self.stack[-1], w))
        elif transition == 'RA':
            w = self.stack.pop()
            self.dependencies.append((self.stack[-1], w))
...
def minibatch_parse(sentences, model, batch_size):
    ...
    pp_sentences = [PartialParse(s) for s in sentences]
    unfinished_parses = copy(pp_sentences)
    while len(unfinished_parses) > 0:
        mini_batch = unfinished_parses[:batch_size]
        while len(mini_batch) > 0:
            pred_transitions = model.predict(mini_batch)
            print(pred_transitions)
            for i, action in enumerate(pred_transitions):
                mini_batch[i].parse_step(action)
            mini_batch = [s for s in mini_batch if len(s.stack) > 1 or len(s.buffer) > 0]
        dependencies += [p.dependencies for p in unfinished_parses[:batch_size]]
